package ca.objectobject.hexdebug.casting.patterns.spells
...
import ca.objectobject.hexdebug.networking.SetLookPitchS2CMsg
...
class OpCongrats : SpellAction {
    ...
    override fun execute(args: List<Iota>, ctx: CastingContext): Triple<RenderedSpell, Int, List<ParticleSpray>> {
        ...
        if (target !is ServerPlayer) throw MishapBadEntity(
            target,
            Component.translatable("text.hexdebug.congrats.player")
        )
    ...
    private data class Spell(val player: ServerPlayer) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            player.sendSystemMessage(Component.translatable("text.hexdebug.congrats", player.displayName));
            HexDebugNetworking.sendToPlayer(player, SetLookPitchS2CMsg(-90f))
        }
    }
}
